...
# I will find the block starting with // ================= TAHFIZ VIEW =================
# and ending at // ================= PENTADBIR VIEW ================= (exclusive)

start_idx = content.find('// ================= TAHFIZ VIEW =================')
end_idx = content.find('// ================= PENTADBIR VIEW =================')

if start_idx != -1 and end_idx != -1:
    new_block = """// ================= TAHFIZ VIEW =================
function TahfizView() {
  return <PenilaianModulView mode="TAHFIZ" />;
}

// ================= AKADEMIK VIEW =================
function AkademikView() {
  return <PenilaianModulView mode="AKADEMIK" />;
}

// ================= PENILAIAN REUSABLE =================
function PenilaianModulView({ mode }: { mode: 'TAHFIZ' | 'AKADEMIK' }) {
  ...
  const isTahfiz = mode === 'TAHFIZ';
  const items = (isTahfiz ? settings.tahfizItems : settings.akademikItems) || [];
  const markahKey = isTahfiz ? 'markahTahfiz' : 'markahAkademik';
  ...
  const evaluatedCandidates = candidates.filter(c => c[markahKey]?.dinilaiOleh === currentUser?.name);
  const currentC = candidates.find(c => c.ic === selectedCandidate);

  useEffect(() => {
    if (currentC && currentC[markahKey]) {
      setMarkah(currentC[markahKey]);
      setCatatan(currentC[markahKey]?.catatan || '');
    } else {
      setMarkah({});
      setCatatan('');
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    updateCandidate(currentC.ic, {
      [markahKey]: {
        ...
        tarikhDinilai: currentC[markahKey]?.tarikhDinilai || new Date().toISOString(),
        catatan
      }
    });
    ...
    <div>
       <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-6 mb-8">
         <div className="flex items-center gap-4">
           <div className={`p-3 rounded-xl ${isTahfiz ? 'bg-emerald-100' : 'bg-blue-100'}`}>
             {isTahfiz ? <FileSignature className={`w-7 h-7 text-emerald-700`} /> : <CheckSquare className={`w-7 h-7 text-blue-700`} />}
           </div>
           <div>
             <h3 className="text-2xl font-extrabold text-slate-900 tracking-tight">Penilaian {isTahfiz ? 'Temuduga (Tahfiz)' : 'Ujian Akademik'}</h3>
       ...
       <div className="flex gap-4 mb-8 border-b border-slate-200 pb-2 overflow-x-auto">
         <button onClick={() => setActiveTab('NILAI')} className={`px-6 py-3 rounded-t-xl font-bold transition-all whitespace-nowrap ${activeTab === 'NILAI' ? (isTahfiz ? 'bg-emerald-600 text-white' : 'bg-blue-600 text-white') : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>1. Menilai Calon</button>
         <button onClick={() => setActiveTab('SENARAI')} className={`px-6 py-3 rounded-t-xl font-bold transition-all whitespace-nowrap ${activeTab === 'SENARAI' ? (isTahfiz ? 'bg-emerald-600 text-white' : 'bg-blue-600 text-white') : 'bg-slate-100 text-slate-500 hover:bg-slate-200'}`}>2. Senarai Calon Dinilai</button>
       </div>
       ...
       {activeTab === 'NILAI' && (
         <div className="max-w-4xl animate-in fade-in">
           <div className="mb-8 bg-slate-50 p-6 rounded-2xl border border-slate-200/60">
             ...
             <select 
               className={`w-full border-2 border-slate-200 rounded-xl px-4 py-3 focus:ring-4 transition-all duration-300 font-medium text-slate-800 bg-white shadow-sm appearance-none ${isTahfiz ? 'focus:ring-emerald-500/20 focus:border-emerald-500' : 'focus:ring-blue-500/20 focus:border-blue-500'}`}
               value={selectedCandidate}
               onChange={(e) => setSelectedCandidate(e.target.value)}
             >
               <option value="">-- Pilih Calon --</option>
               {pendingCandidates.map(c => (
                 <option key={c.id} value={c.ic}>{c.name} ({c.ic}) {c[markahKey] ? '- (Draf)' : ''}</option>
               ))}
             </select>
           ...
           {currentC && (
             <div className={`${isTahfiz ? 'bg-emerald-50/50 border-emerald-100 shadow-emerald-100/30' : 'bg-blue-50/50 border-blue-100 shadow-blue-100/30'} rounded-[2rem] p-8 border-2 animate-in fade-in slide-in-from-top-4 shadow-xl`}>
               <div className={`mb-8 bg-white p-6 rounded-2xl shadow-sm border ${isTahfiz ? 'border-emerald-100/50' : 'border-blue-100/50'} flex flex-col sm:flex-row items-center sm:items-start gap-6 text-center sm:text-left`}>
                 {currentC.gambarUrl ? (
               ...
               <form onSubmit={handleSubmit} className="space-y-6">
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                    {items.map((item) => (
                        <div key={item.id}>
                          ...
                          <input type="number" max={item.weight} min="0" required value={markah[item.id] !== undefined ? markah[item.id] : ''} onChange={e=>setMarkah({...markah, [item.id]: Number(e.target.value)})} className={`w-full p-4 rounded-xl border-2 bg-white font-bold text-lg text-slate-800 transition-all ${isTahfiz ? 'border-emerald-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20' : 'border-blue-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20'}`} />
                        </div>
                    ))}
                    <div>
                  ...
                  <div className="pt-2">
                    ...
                    <textarea 
                      ...
                      className={`w-full p-4 rounded-xl border-2 bg-white font-medium text-slate-700 transition-all ${isTahfiz ? 'border-emerald-200 focus:border-emerald-500 focus:ring-4 focus:ring-emerald-500/20' : 'border-blue-200 focus:border-blue-500 focus:ring-4 focus:ring-blue-500/20'}`} 
                    />
                  </div>

                  <div className="pt-6 flex justify-end">
                    <button type="submit" className={`text-white px-8 py-4 rounded-xl font-bold shadow-lg transition-all duration-300 hover:scale-[1.02] active:scale-[0.98] text-lg w-full sm:w-auto ${isTahfiz ? 'bg-emerald-600 hover:bg-emerald-700 shadow-emerald-600/30' : 'bg-blue-600 hover:bg-blue-700 shadow-blue-600/30'}`}>Simpan Maklumat Penilaian</button>
                  </div>
               </form>
             </div>
           )}
               ...
               <table className="min-w-full divide-y divide-slate-200">
                 <thead className="bg-slate-50">
                   <tr>
                     ...
                     <th className={`px-6 py-4 text-center text-xs font-bold uppercase tracking-widest ${isTahfiz ? 'text-emerald-700 bg-emerald-50' : 'text-blue-700 bg-blue-50'}`}>Jumlah</th>
                 ...
                 <tbody className="bg-white divide-y divide-slate-100">
                   {evaluatedCandidates.length === 0 ? (
                     ...
                     evaluatedCandidates.map(c => {
                       const canEdit = isSameDay(c[markahKey]?.tarikhDinilai);
                       return (
                         <tr key={c.id} className="hover:bg-slate-50 transition-colors">
                           ...
                           <td className="px-6 py-4">
                             ...
                             <div className="text-xs text-slate-400 mt-1">{new Date(c[markahKey]?.tarikhDinilai || '').toLocaleDateString('ms-MY')}</div>
                           </td>
                           <td className="px-6 py-4">
                             <p className="text-sm text-slate-600 line-clamp-3" title={c[markahKey]?.catatan}>{c[markahKey]?.catatan || '-'}</p>
                           </td>
                           <td className={`px-6 py-4 whitespace-nowrap font-extrabold text-2xl text-center ${isTahfiz ? 'text-emerald-600 bg-emerald-50/30' : 'text-blue-600 bg-blue-50/30'}`}>{c[markahKey]?.jumlah}</td>
                           <td className="px-6 py-4 whitespace-nowrap text-center">
                             {canEdit ? (
                               <button 
                                 onClick={() => handleEdit(c.ic)} 
                                 className={`px-4 py-2 font-bold text-sm rounded-lg transition-colors border shadow-sm ${isTahfiz ? 'bg-emerald-50 hover:bg-emerald-100 text-emerald-700 border-emerald-200' : 'bg-blue-50 hover:bg-blue-100 text-blue-700 border-blue-200'}`}
                               >
                                 Kemaskini
                               </button>
                             ) : (
...
\n"""
    ...
    with open('src/components/dashboard/AdminPanel.tsx', 'w') as out_f:
        out_f.write(content)
